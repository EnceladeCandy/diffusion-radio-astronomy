...
from utils import link_function, probes_256, fits_to_tensor
...
def main(args):
    ...
    # Args of the scripts
    if N_WORKERS>1: # Array job case
        idx = THIS_WORKER
        num_samples = args.num_samples
        num_iters = args.num_iters
        if args.sampler.lower() == "pc":
            ...
            if idx%100==0:    
                os.mkdir(folder_dir)
    else: # Simple job case
        idx = args.idx
        num_samples = args.num_samples
        num_iters = args.num_iters
        if args.sampler.lower() == "pc": # Corrector steps and snr needed only for the pc sampler
